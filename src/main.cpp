...
#if defined(NDEBUG)
# error "Oximcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc6aba0564a937eb0600d1bcbdc70cc2c704d611b089a65bf9dca715d47947ad0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Oximcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Oximcoin Signed Message:\n";
...
td::map<uint256,CCoins>::iterator CCoinsViewCache::FetchCoins(const uint256 &txid) {
...
{
    // Oximcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 7191.78082 * COIN;
	
    // Subsidy is cut in half every 43800 blocks, which will occur approximately every 1 month
    nSubsidy >>= (nHeight / 43800); // Oximcoin: 43800 blocks in ~1 month

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 60; // Oximcoin: 1 minute
static const int64 nTargetSpacing = 1 * 60; // Oximcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Oximcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Oximcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc6aba0564a937eb0600d1bcbdc70cc2c704d611b089a65bf9dca715d47947ad0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 20/Jan/2016 California Said to Target Exxon in Climate Inquiry";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047381910fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1453298530;
        ...
        assert(block.hashMerkleRoot == uint256("0x52c204760a2f323baa5e8a9553eee733cc5a0a4211323bc811dd5105da61a5c1"));
...
extern mapuint256, CAlert> mapAlerts;
...
unsigned char pchMessageStart[4] = { 0xa1, 0xf5, 0xc3, 0xd4 }; // Oximcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OximcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OximcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OximcoinMiner(CWallet *pwallet)
{
    printf("OximcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("oximcoin-miner");
    ...
    try { loop {
        ...
        printf("Running OximcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OximcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OximcoinMiner, pwallet));
}
